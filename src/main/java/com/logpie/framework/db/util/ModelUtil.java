...
public class ModelUtil {
	 ...
	 *         is java entity
	 */
	public static List<KVP> getModelKVP(final LogpieModel model,
			final boolean hasAutogeneratedKey) {
		List<KVP> result = new ArrayList<KVP>();
		...
		for (Field field : fields) {
			...
			if (field.isAnnotationPresent(Column.class)) {
				...
				result.add(new KVP(column.name(), value));
			}
			// find every field of model corresponding foreign key
				...
				if (entity == null) {
					result.add(new KVP(column.name(), null));
				} else {
					// get fields of table
					...
					for (Field f : referencedFields) {
						...
						if (f.isAnnotationPresent(ID.class)) {
							result.add(new KVP(column.name(), runGetter(f,
									entity)));
							break;
						}
					}
	...
	public static Long getIdValue(final LogpieModel model) {
		Field[] fields = model.getClass().getDeclaredFields();
		for (Field field : fields) {
