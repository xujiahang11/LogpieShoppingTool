...
public class DatabaseUtil {
	...
	public static List<String> getBasicColumns(final Class<?> c,
		...
		for (Column column : columns) {
			res.add(getTableName(c) + "." + column.name());
		}
		return res;
	}

	/**
	 * get columns of Class c
	 * 
	 * @param c
	 * @param hasAutogeneratedKey
	 * @return column names with table alias or name
	...
	public static List<String> getColumns(final Class<?> c,
			final boolean hasAutogeneratedKey) {
		List<String> res = new ArrayList<String>();
		res.addAll(getBasicColumns(c, hasAutogeneratedKey));

		List<ForeignEntity> foreignEntities = getForeignEntityAnnotations(c);
		for (ForeignEntity entity : foreignEntities) {
			if (entity.referencedTableAlias().equals("")) {
				res.add(getTableName(entity.referencedTable()) + "."
						+ entity.name());
			} else {
				res.add(entity.referencedTableAlias() + "." + entity.name());
			}
		}
		return res;
	}
	 ...
	 * @return column names with table alias or name
	 */
	public static List<String> getLinkedBasicColumns(final Class<?> c) {
		List<String> res = new ArrayList<String>();
		...
		for (ForeignEntity entity : foreignEntities) {
			...
			for (Column key : referencedKeys) {
				if (entity.referencedTableAlias().equals("")) {
					...
					res.add(entity.referencedTableAlias() + "." + key.name());
				}
			}
		}
	...
	private static String toSqlString(final Class<?> c, final String key,
			final Object obj) {
